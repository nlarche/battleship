...
describe("Build grid with boat", () => {
  it("should a boat of size 1", () => {
    ...
    expect(grid.select("A1")).toStrictEqual(
      buildCellWithBoatFromPosition("A1")
    );
  });
  ...
  it("should had a boat of size 2 horizontaly", () => {
    ...
    expect(grid.select("A2")).toStrictEqual(
      buildCellWithBoatFromPosition("A2")
    );
  ...
  it("should had a boat of size 2 verticaly", () => {
    ...
    expect(grid.select("B1")).toStrictEqual(
      buildCellWithBoatFromPosition("B1")
    );
  ...
  it("should had a boat of size 3 verticaly from C6", () => {
    ...
    expect(grid.select("C6")).toStrictEqual(
      buildCellWithBoatFromPosition("C6")
    );
    expect(grid.select("D6")).toStrictEqual(
      buildCellWithBoatFromPosition("D6")
    );
    expect(grid.select("E6")).toStrictEqual(
      buildCellWithBoatFromPosition("E6")
    );
    expect(grid.select("A2")).toStrictEqual(buildEmptyCell("A2"));
    expect(grid.select("B2")).toStrictEqual(buildEmptyCell("B2"));
  });
  ...
  describe("shoot a cell", () => {
    it("should select the right cell", () => {
      const grid = new Grid(6);
      const cell = grid.select("B3") as Cell;
      expect(cell).toBeDefined;
      grid.shoot(cell.position);
      expect(grid.select("B3")).toStrictEqual(toshootedCell(cell));
    });
  });
});
